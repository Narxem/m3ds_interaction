...
void GLApplication::updateCamera() {
    if (left()) {
        _camera.translate(-.1, 0., 0., Coordinate_Local);
    }
    if (right()) {
        _camera.translate(.1, 0., 0., Coordinate_Local);
    }
    if (forward()) {
        _camera.translate(0., 0., -.1, Coordinate_Local);
    }
    if (backward()) {
        _camera.translate(0., 0., .1, Coordinate_Local);
    }
    if (mouseLeft()) {
        ...
        _camera.rotate(-deltaMouseX() / 3, _camera.directionTo(Coordinate_Local, Vector3(0., 1., 0.)), Coordinate_Local);
        _camera.rotate(deltaMouseY() / 3, Vector3(1., 0., 0.), Coordinate_Local);
    }
}
